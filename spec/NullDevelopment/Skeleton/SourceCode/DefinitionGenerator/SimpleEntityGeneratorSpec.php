...
class SimpleEntityGeneratorSpec extends ObjectBehavior
{
    ...
    {
        $this->shouldHaveType(SimpleEntityGenerator::class);
        $this->shouldImplement(DefinitionGenerator::class);
    }

    public function it_supports_single_value_object(SimpleEntity $definition)
    {
        $this->supports($definition)->shouldReturn(true);
    }

    public function it_creates_class_code(SimpleEntity $definition)
    {
    ...
    public function it_creates_class_code_with_parent(SimpleEntity $definition)
    {
        $definition->getClassName()->shouldBeCalled()->willReturn('UserEntity');
        $definition->getNamespace()->shouldBeCalled()->willReturn('MyVendor\\WebShop');
    ...
    public function it_creates_class_code_with_interfaces(SimpleEntity $definition, InterfaceName $interfaceName)
    {
        $definition->getClassName()->shouldBeCalled()->willReturn('UserEntity');
        $definition->getNamespace()->shouldBeCalled()->willReturn('MyVendor\\WebShop');
        $definition->hasParent()->shouldBeCalled()->willReturn(false);
    ...
    public function it_creates_class_code_with_properties(SimpleEntity $definition, Property $firstName)
    {
        $definition->getClassName()->shouldBeCalled()->willReturn('UserEntity');
        $definition->getNamespace()->shouldBeCalled()->willReturn('MyVendor\\WebShop');
        $definition->hasParent()->shouldBeCalled()->willReturn(false);
