...
class GitHubBranchTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->commit = new GitHubCommit(new CommitSha('sha'), new CommitMessage('message'), new CommitDate('2018-01-01 00:01:00'), new CommitAuthor(new AuthorName('name'), new EmailAddress('value'), new CommitDate('2018-01-01 00:01:00'), new CommitAuthorDetails(new UserId(1), new UserLogin('login'), new AccountType('type'), new UserAvatarUrl('avatarUrl'), new GravatarId('value'), new UserHtmlUrl('htmlUrl'), new UserApiUrl('apiUrl'), true)), new CommitCommitter(new CommitterName('name'), new EmailAddress('value'), new CommitDate('2018-01-01 00:01:00'), new CommitCommitterDetails(new UserId(1), new UserLogin('login'), new AccountType('type'), new UserAvatarUrl('avatarUrl'), new GravatarId('value'), new UserHtmlUrl('htmlUrl'), new UserApiUrl('apiUrl'), true)), new CommitTree(new TreeSha('sha'), new TreeUrl('url')), new CommitParentCollection([new CommitParent(new CommitSha('sha'), new ParentApiUrl('apiUrl'), new ParentHtmlUrl('htmlUrl'))]), new CommitVerification(new VerificationVerified(true), new VerificationReason('reason'), new VerificationSignature('signature'), new VerificationPayload('payload')), new CommitApiUrl('apiUrl'), new CommitHtmlUrl('htmlUrl'));
        $this->protected = true;
